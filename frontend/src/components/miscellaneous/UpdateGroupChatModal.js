...
const UpdateGroupChatModal = ({ fetchAgain, setFetchAgain }) => {
  ...
  const handleRename = async () => {
    ...
    try {
      ...
      toast({ title: "Error Occurred!", description: errorMessage, status: "error", duration: 5000, isClosable: true, position: "bottom" });
    } finally {
  ...
  useEffect(() => {
    if (selectedChat && user) setGroupChatName(selectedChat.chatName);
  }, [selectedChat, user]);
